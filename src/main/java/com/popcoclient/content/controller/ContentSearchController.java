...
public class ContentSearchController {
    ...
    public ResponseEntity<ApiResponse<Page<ContentDocument>>> advancedSearch(
            @RequestParam(required = false) String keyword,
            @RequestParam(required = false) String contentType,
            @RequestParam(required = false) List<String> actors,
            @PageableDefault(size = 20) Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.success(searchService.advancedSearch(keyword, contentType, actors, pageable)));
    }
