...
### 시연 영상
<a href="https://youtu.be/TDXbO8EXBr8" target="_blank">
  <img src="./img/popco_video.png" alt="팝코(PopCo) 프로젝트 시연 영상" width="500"/>
</a>

### POPCO 발표 자료
[📄프로젝트 발표](./유레카_최종융합프로젝트_발표.pdf)
...
- **개인 맞춤 추천2 (아이템 필터링)**:
    - 사용자 피드백('좋아요', '싫어요', 별점)을 기반으로 아이템 기반 필터링 적용
...
- **페르소나 진단 및 추천 (협업 필터링)**:
    ...
    - **초기 진단**:
        ...
        - '아기 POPCO' / '어른 POPCO' 캐릭터로 취향 선명도 구분
    - **실시간 반영**:
...
- **주간 선착순 퀴즈 이벤트**:
    - 실시간으로 진행되는 선착순 퀴즈 이벤트
    - 동시성 제어(Lua Script + Redis)를 이용한 대규모 데이터 처리
    - 이벤트 시작 10분전부터 팝업 알람 자동 수신 기능
...
|  이름  | 역할 | 담당 업무                                                                                                       | GitHub | 
| ---- | ---- |-------------------------------------------------------------------------------------------------------------| ---- |
| 박기환 | 팀장 | - 프로젝트 총괄<br> - 개발 환경 구성<br> - 사용자 통계 API<br> - 실시간 퀴즈 이벤트와 동시성 처리<br> -SSE 구현<br> - 아이템 기반 추천 알고리즘 (팝코리즘)  | https://github.com/GihwanPark |
| 김건택 | 팀원 | - 배포 환경 구성 ( CI/CD 등 더 자세히 어필해주세요 ) <br>                                                                    | 깃 주소 |
| 변지민 | 팀원 | - 페르소나 기반 추천 알고리즘<br> - Elasticsearch 필터링 구현<br> - 위시리스트 API 구현<br> - 전체 콘텐츠 조회(기본, 최신순, 인기순) API 구현<br> - FastAPI 활용  | https://github.com/minschive |
| 신혜원 | 팀원 | - 배치 스케줄링<br> - 리뷰, 신고, 인증 API<br> - jwt, 카카오 로그인 구현<br> - OpenAI, gemini 활용<br> - 콘텐츠 기반 추천 알고리즘(1등 연관 작품) | https://github.com/hyew0nn |
...
## 협업
* **Jira, Figma, Slack, Discord, Notion**
